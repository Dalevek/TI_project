(function(){
  ...
  function bookapp(msg) {
    // Wczytanie stylów CSS.
    var css = jQuery('<link>');
    css.attr({
      rel: 'stylesheet',
      type: 'text/css',
      href: static_url + 'css/bookapp.css?r=' + Math.floor(Math.  random()*99999999999999999999)
    });
    jQuery('head').append(css);

    // Wczytanie kodu HTML.
    box_html = '<div id="bookapp"><a href="#" id="close">&times;</a><h1>Wybierz obraz do dodania:</h1><div class="images"></div></div>';
    jQuery('body').append(box_html);

    // Zdarzenie close.
    jQuery('#bookapp #close').click(function(){
       jQuery('#bookapp').remove();
    });
    ...
    jQuery.each(jQuery('img[src$="jpg"]'), function(index, image) {
      if (jQuery(image).width() >= min_width && jQuery(image).height() >=     min_height)
      {
        ...
        jQuery('#bookapp .images').append('<a href="#"><img src="'+     image_url +'" /></a>');
      }
    });

    // Po zaznaczeniu obrazu przejdź pod podany adres URL wraz z obrazem.
    jQuery('#bookapp .images a').click(function(e){
